...
async function getUserRoleDict() {
...
function changeSelectFilter(value: typeof selectFilterValues.value) {
...
function changeTreeFilter(val: string[]) {
  ElMessage.success('请注意查看请求参数变化 🤔')
  proTable.value!.pageable.pageNum = 1
...
// 删除用户信息
async function deleteAccount(params: User.ResUserList) {
  await useHandleData(deleteUser, { id: [params.id] }, `删除【${params.username}】用户`)
  proTable.value?.getTableList()
}

// 重置用户密码
async function resetPass(params: User.ResUserList) {
...
function openDrawer(title: string, row: Partial<User.ResUserList> = {}) {
  const params = {
    title,
    isView: title === '查看',
    row: { ...row },
    ...
    <div class="table-box">
      ...
      >
        ...
        <template #tableHeader>
          ...
          <el-button type="primary" :icon="Pointer" plain @click="setCurrent">
            选中第四行
          </el-button>
        </template>
        <!-- 单选 -->
